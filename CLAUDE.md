...
test/             # aasa, shows (integration via workerd), format, poster (unit), helpers (shared)
```
...
- **Validate before caching.** Only a 200 whose body decodes into a Concert is written to `caches.default` — a junk 200 must render the degraded page WITHOUT being cached, or one bad response pins five minutes of 502s per colo after upstream recovers. A cached entry that stops decoding is deleted and refetched, not served as an error. The poisoning-recovery test in `test/shows.test.ts` pins this ordering.
