...
- **The OG card is generated, not edited.** `assets/og-card.png` and `src/og-card-data.ts` come from `scripts/generate-og-card.swift` + `scripts/embed-og-card.mjs` (macOS). The per-show OG image (mockup variant B) is ticket #2 — don't build image generation into this Worker ad hoc.
